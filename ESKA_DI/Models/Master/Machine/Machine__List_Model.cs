...
{
    public class ListFindParamMachine
    {
    ...
    public class Machine__List_Model
    {
        static string ViewSql = "SELECT *" +
                                "FROM \"Tm_Machine\" T0 " +
                                "ORDER BY T0.\"CreatedDate\" DESC";

        public static void SetBindingData(GridViewModel state, int userId, ListFindParamMachine cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<MachineView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamMachine param, string sqlCriteria)
        {
        ...
        public static List<MachineView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamMachine param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<MachineView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<MachineView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<MachineView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<MachineView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<MachineView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                MachineView___ view = new MachineView___();
                views.Add(view);
            }
        ...
        public static GridViewSettings CreateExportGridViewSettings()
        {
            ...
            settings.Columns.Add("MachineCode", "Machine Code");
            settings.Columns.Add("MachineName", "Machine Name");
            settings.Columns.Add("Description", "Description");
