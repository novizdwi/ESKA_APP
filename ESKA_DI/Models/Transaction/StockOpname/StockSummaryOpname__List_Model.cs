...
{
    public class ListFindParamStockSummaryOpname
    {
    ...
    public class StockSummaryOpnameView___
    {
    ...
    public class StockSummaryOpname__List_Model
    {
        static string ViewSql = "SELECT T0.*" +
            "FROM \"Tx_StockSummaryOpname\" T0 " +
        "ORDER BY T0.\"CreatedDate\" DESC";

        public static void SetBindingData(GridViewModel state, int userId, ListFindParamStockSummaryOpname cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<StockSummaryOpnameView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamStockSummaryOpname param, string sqlCriteria)
        {
        ...
        public static List<StockSummaryOpnameView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamStockSummaryOpname param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<StockSummaryOpnameView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<StockSummaryOpnameView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<StockSummaryOpnameView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<StockSummaryOpnameView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<StockSummaryOpnameView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                StockSummaryOpnameView___ view = new StockSummaryOpnameView___();
                views.Add(view);
            }
