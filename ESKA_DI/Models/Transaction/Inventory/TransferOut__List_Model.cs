...
{
    public class ListFindParamTransferOut
    {
    ...
    public class TransferOutView___
    {
    ...
    public class TransferOut__List_Model
    {
        static string ViewSql = "SELECT * " +
                                ...
                                "LEFT JOIN ( " +
                                    "SELECT " +
                                    "   T0.\"TransNo\" AS \"SummaryTransNo_\" , " +
                                    "   T1.\"BaseId\" " +
                                    "FROM \"Tx_TransferSummaryOut\" T0 " +
                                    "INNER JOIN \"Tx_TransferSummaryOut_Ref\" T1 ON T0.\"Id\" = T1.\"Id\" " +
                                    "WHERE T0.\"Status\" NOT IN ('Cancel') " +
                                ") T2_ ON T0.\"Id\" = T2_.\"BaseId\"  " +

                                "ORDER BY T0.\"CreatedDate\" DESC";

        public static void SetBindingData(GridViewModel state, int userId, ListFindParamTransferOut cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<TransferOutView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamTransferOut param, string sqlCriteria)
        {
        ...
        public static List<TransferOutView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamTransferOut param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<TransferOutView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<TransferOutView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<TransferOutView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<TransferOutView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<TransferOutView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                TransferOutView___ view = new TransferOutView___();
                views.Add(view);
            }
