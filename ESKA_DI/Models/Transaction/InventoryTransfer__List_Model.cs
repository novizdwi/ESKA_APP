...
{
    public class ListFindParamInventoryTransfer
    {
    ...
    public class InventoryTransferView___
    {
    ...
    public class InventoryTransfer__List_Model
    {
        static string ViewSql = "SELECT *" +
                                "FROM \"Tx_InventoryTransfer\" T0 " +
                                "ORDER BY T0.\"CreatedDate\" DESC";

        public static void SetBindingData(GridViewModel state, int userId, ListFindParamInventoryTransfer cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<InventoryTransferView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamInventoryTransfer param, string sqlCriteria)
        {
        ...
        public static List<InventoryTransferView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamInventoryTransfer param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<InventoryTransferView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<InventoryTransferView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<InventoryTransferView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<InventoryTransferView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<InventoryTransferView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                InventoryTransferView___ view = new InventoryTransferView___();
                views.Add(view);
            }
