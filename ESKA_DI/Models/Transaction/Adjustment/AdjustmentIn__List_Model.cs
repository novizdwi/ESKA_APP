...
{
    public class ListFindParamAdjustmentIn
    {
    ...
    public class AdjustmentInView___
    {
    ...
    public class AdjustmentIn__List_Model
    {
        ...
        public static void SetBindingData(GridViewModel state, int userId, ListFindParamAdjustmentIn cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<AdjustmentInView___> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, ListFindParamAdjustmentIn param, string sqlCriteria)
        {
        ...
        public static List<AdjustmentInView___> GetDataList(HANA_APP CONTEXT, int userId, ListFindParamAdjustmentIn param, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var views = new List<AdjustmentInView___>();
            ...
            if (param != null)
            {
                if (param.IsFindTransDate)
                {
                    ...
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 AND \"TransDate\"<=:p1 ";
                        views = CONTEXT.Database.SqlQuery<AdjustmentInView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date, param.TransDate_To.Value.Date).ToList();
                    }
                    else if (param.TransDate_From != null)
                    {
                        //ssql = ssql + " AND \"TransDate\">=:p0 ";
                        views = CONTEXT.Database.SqlQuery<AdjustmentInView___>(ssql + sqlSort + ssqlLimit, param.TransDate_From.Value.Date).ToList();
                    }
                    else if (param.TransDate_To != null)
                    {
                        //ssql = ssql + " AND \"TransDate\"<=:p0 ";
                        views = CONTEXT.Database.SqlQuery<AdjustmentInView___>(ssql + sqlSort + ssqlLimit, param.TransDate_To.Value.Date).ToList();
                    }
                }
                else
                {
                    views = CONTEXT.Database.SqlQuery<AdjustmentInView___>(ssql + sqlSort + ssqlLimit).ToList();
                }
            }
            else
            {
            ...
            if (views.Count == 0)
            {
                AdjustmentInView___ view = new AdjustmentInView___();
                views.Add(view);
            }
