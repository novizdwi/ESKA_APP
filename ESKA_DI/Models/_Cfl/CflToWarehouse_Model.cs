...
{
    public class CflToWarehouse_ParamModel
    {
    ...
    public class CflToWarehouse_View__
    {
        public string WhsCode { get; set; }
        public string WhsName { get; set; }
    }

    public class CflToWarehouse_Model
    {
        ...
        public static void SetBindingData(GridViewModel state, int userId, CflToWarehouse_ParamModel cflParam)
        {
        ...
        public static void GetData(GridViewCustomBindingGetDataArgs e, List<CflToWarehouse_View__> dataList)
        {
            e.Data = dataList;
        }

        public static int GetRowCount(HANA_APP CONTEXT, int userId, CflToWarehouse_ParamModel cflParam, string sqlCriteria)
        {

            var Cfl_Sql = CflToWarehouse_Model.ssql;
        ...
        public static List<CflToWarehouse_View__> GetDataList(HANA_APP CONTEXT, int userId, CflToWarehouse_ParamModel cflWarehouseParam, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var CflToWarehouses_ = GetDataList(CONTEXT, userId, sqlCriteria, sqlSort, PageIndex, PageSize);

            if (CflToWarehouses_.Count == 0)
            {
                CflToWarehouse_View__ item = new CflToWarehouse_View__();
                CflToWarehouses_.Add(item);
            }


            return CflToWarehouses_;

        }

        public static List<CflToWarehouse_View__> GetDataList(HANA_APP CONTEXT, int userId, string sqlCriteria, string sqlSort, int PageIndex, int PageSize)
        {
            ...
            var items = CONTEXT.Database.SqlQuery<CflToWarehouse_View__>(ssql + sqlSort + ssqlLimit).ToList();
        ...
        public static GridViewSettings CreateExportGridViewSettings(CflToWarehouse_ParamModel cflWarehouseParam)
        {
