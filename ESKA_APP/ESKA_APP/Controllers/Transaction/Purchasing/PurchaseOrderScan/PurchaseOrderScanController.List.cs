...
namespace Controllers.Transaction.Purchasing
{
    ...
    {
        public ListFindParamPurchaseOrderScan GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamPurchaseOrderScan();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvPurchaseOrderScanList");
            if (viewModel == null)
            {
                viewModel = PurchaseOrderScan__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamPurchaseOrderScan param)
        {

            PurchaseOrderScan__List_Model.SetBindingData(viewModel, userId, param);
