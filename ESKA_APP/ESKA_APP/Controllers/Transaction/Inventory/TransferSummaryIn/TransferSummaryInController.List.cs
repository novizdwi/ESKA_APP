...
namespace Controllers.Transaction.Inventory
{
    ...
    {
        public ListFindParamTransferSummaryIn GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamTransferSummaryIn();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvTransferSummaryInList");
            if (viewModel == null)
            {
                viewModel = TransferSummaryIn__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamTransferSummaryIn param)
        {

            TransferSummaryIn__List_Model.SetBindingData(viewModel, userId, param);
