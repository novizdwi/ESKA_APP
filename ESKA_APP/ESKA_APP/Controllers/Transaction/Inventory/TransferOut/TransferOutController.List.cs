...
namespace Controllers.Transaction.Inventory
{
    ...
    {
        public ListFindParamTransferOut GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamTransferOut();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvTransferOutList");
            if (viewModel == null)
            {
                viewModel = TransferOut__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamTransferOut param)
        {

            TransferOut__List_Model.SetBindingData(viewModel, userId, param);
