...
namespace Controllers.Transaction.Inventory
{
    ...
    {
        public ListFindParamTransferRequest GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamTransferRequest();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvTransferRequestList");
            if (viewModel == null)
            {
                viewModel = TransferRequest__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamTransferRequest param)
        {

            TransferRequest__List_Model.SetBindingData(viewModel, userId, param);
