...
namespace Controllers.Transaction.Inventory
{
    ...
    {
        public ListFindParamTransferIn GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamTransferIn();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvTransferInList");
            if (viewModel == null)
            {
                viewModel = TransferIn__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamTransferIn param)
        {

            TransferIn__List_Model.SetBindingData(viewModel, userId, param);
