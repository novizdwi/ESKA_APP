...
namespace Controllers.Transaction.Adjustment
{
    ...
    {
        public ListFindParamAdjustmentOut GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamAdjustmentOut();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvAdjustmentOutList");
            if (viewModel == null)
            {
                viewModel = AdjustmentOut__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamAdjustmentOut param)
        {

            AdjustmentOut__List_Model.SetBindingData(viewModel, userId, param);
