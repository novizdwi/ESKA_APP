...
namespace Controllers.Transaction.Adjustment
{
    public partial class AdjustmentOutController : BaseController
    {
        ...
        public ActionResult Attachment_Upload()
        {
            ...
            //var UploadMultiFile = UploadControlExtension.GetUploadedFiles("UploadMultiFile", AdjustmentOutUploadControlHelper.ValidationSettings, AdjustmentOutUploadControlHelper.FileUploadComplete);
            ...
            if (UploadMultiFile != null)
            {
                ...
                List<AdjustmentOut_AttachmentModel> listModel = new List<AdjustmentOut_AttachmentModel>();
                ...
                for (int i = 0; i < UploadMultiFile.Length; i++)
                {
                    if (UploadMultiFile[i].FileBytes.Length > 0 && UploadMultiFile[i].IsValid)
                    {
                        ...
                        AdjustmentOut_AttachmentModel model = new AdjustmentOut_AttachmentModel();
                ...
                adjustmentOutService.Detail_Add(listModel);
                ...
                for (int i = 0; i < listModel.Count; i++)
                {
                    ...
                    //string strFilename = AdjustmentOutUploadControlHelper.GetFilePath(listModel[i].Guid + "_" + listModel[i].FileName);
        ...
        public FileResult Attachment_Download(int DetId = 0)
        {
            ...
            using (var CONTEXT = new HANA_APP()) {
                Tx_AdjustmentOut_Attachment model = CONTEXT.Tx_AdjustmentOut_Attachment.Find(DetId);
                if (model != null)
                {
                    ...
                    //return File(AdjustmentOutUploadControlHelper.GetFilePath(model.Guid + "_" + model.FileName), System.Net.Mime.MediaTypeNames.Application.Octet, fileName);
    ...
    //public class AdjustmentOutUploadControlHelper
    //{
    //    public const string UploadDirectory = "~/Content/Attachment/AdjustmentOut/";
