...
namespace Controllers.Transaction.Adjustment
{
    ...
    {
        public ListFindParamAdjustmentIn GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamAdjustmentIn();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvAdjustmentInList");
            if (viewModel == null)
            {
                viewModel = AdjustmentIn__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamAdjustmentIn param)
        {

            AdjustmentIn__List_Model.SetBindingData(viewModel, userId, param);
