...
namespace Controllers.Transaction.StockOpname
{
    public partial class RequestStockOpnameController : BaseController
    {
        ...
        public ActionResult CancelReason_PopupListOnDemandPartial(long Id = 0)
        {
            ...
            RequestStockOpnameModel model;

            if (Id != 0)
            {
                model = requestStockOpnameService.GetById(userId, Id);
            }
            else
            {
                model = requestStockOpnameService.GetNewModel(userId);
            }
        ...
        {
            int userId = (int)Session["userId"];
            var model = new RequestStockOpnameModel();
