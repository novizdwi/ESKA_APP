...
namespace Controllers.Transaction.StockOpname
{
    ...
    {
        public ListFindParamStockSummaryOpname GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamStockSummaryOpname();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvStockSummaryOpnameList");
            if (viewModel == null)
            {
                viewModel = StockSummaryOpname__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamStockSummaryOpname param)
        {

            StockSummaryOpname__List_Model.SetBindingData(viewModel, userId, param);
