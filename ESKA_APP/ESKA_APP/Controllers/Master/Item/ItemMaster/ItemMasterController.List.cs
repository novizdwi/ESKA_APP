...
namespace Controllers.Master.Item
{
    ...
    {
        public ListFindParamItemMaster GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamItemMaster();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvItemMasterList");
            if (viewModel == null)
            {
                viewModel = ItemMaster__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamItemMaster param)
        {

            ItemMaster__List_Model.SetBindingData(viewModel, userId, param);
