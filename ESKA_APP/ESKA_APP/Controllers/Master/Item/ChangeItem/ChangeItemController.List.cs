...
namespace Controllers.Master.Item
{
    ...
    {
        public ListFindParamChangeItem GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamChangeItem();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvChangeItemList");
            if (viewModel == null)
            {
                viewModel = ChangeItem__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamChangeItem param)
        {

            ChangeItem__List_Model.SetBindingData(viewModel, userId, param);
