...
namespace Controllers.Master.Item
{
    ...
    {
        public ListFindParamDeactiveTag GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamDeactiveTag();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvDeactiveTagList");
            if (viewModel == null)
            {
                viewModel = DeactiveTag__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamDeactiveTag param)
        {

            DeactiveTag__List_Model.SetBindingData(viewModel, userId, param);
