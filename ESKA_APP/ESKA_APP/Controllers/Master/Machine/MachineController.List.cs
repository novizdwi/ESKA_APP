...
namespace Controllers.Master
{
    ...
    {
        public ListFindParamMachine GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamMachine();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvMachineList");
            if (viewModel == null)
            {
                viewModel = Machine__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamMachine param)
        {

            Machine__List_Model.SetBindingData(viewModel, userId, param);
