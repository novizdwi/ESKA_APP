...
namespace Controllers.Master
{
    ...
    {
        public ListFindParamPosition GetParam(HttpRequestBase Request)
        {

            var param = new ListFindParamPosition();
        ...
        {
            var viewModel = GridViewExtension.GetViewModel("gvPositionList");
            if (viewModel == null)
            {
                viewModel = Position__List_Model.CreateGridViewModel();
            }

            return viewModel;
        }

        static void ProcessCustomBinding(int userId, GridViewModel viewModel, ListFindParamPosition param)
        {

            Position__List_Model.SetBindingData(viewModel, userId, param);
